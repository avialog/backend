...
func (p *PDFExporter) logBookRow(record SingleLogbookEntry) {
	...
	p.totalPage = CalculateTotals(p.totalPage, record)
	p.totalTime = CalculateTotals(p.totalTime, record)
...
func (p *PDFExporter) printA4LogbookBody(record SingleLogbookEntry) {
	fill := p.isFillLine()
